...
def mypres():
    conn = MySQLdb.connect(user="root", passwd="rishi26071997", db="rishi")
    cur = conn.cursor()
    t1 = str(session['username'])
    cur.execute("select docname,detail from prescrip where patname=\""+t1+"\";")
    row = cur.fetchall()
    conn.commit()
    cur.close()
    conn.close()
    #in this we will go to prescription page where doctor will assign some instruction for patient
    return render_template('mypres.html',data=row)

@app.route('/subpres/<string:patid>', methods=['GET','POST'])
def subpres(patid):
    det =str(request.form['pr'])
    doc=str(session['username'])
    pat=str(patid)
    print(det)
    print(doc)
    print(pat)
    conn = MySQLdb.connect(user="root", passwd="rishi26071997", db="rishi")
    cur = conn.cursor()
    cur.execute("insert into prescrip values (\""+ pat+"\",\""+ doc+"\",\""+ det+"\");")
    cur.execute("select userid from Patient;")
    row = cur.fetchall()
    conn.commit()
    cur.close()
...
@app.route('/pres/<string:patid>', methods=['GET','POST'])
def pres(patid):
    return render_template('pres.html', id=patid)
